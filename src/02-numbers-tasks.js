 ...
 *   0    => 0
 */
const getCircleCircumference = (radius) => (radius * 2) * 3.14159265358979323846;
 ...
 *   5*x = 0         => 0
 */
const getLinearEquationRoot = (a, b) => (b / a) * -1;
 ...
 *   (0,1) (1,2)     => 0
 */
const getAngleBetweenVectors = (x1, y1, x2, y2) => Math.abs(Math.atan2(x2, y2)
  - Math.atan2(x1, y1));
...
const roundToPowerOfTen = (num, pow) => {
  ...
  const insert = [...split.slice(0, -pow), '.', ...split.slice(-pow)];
  return !pow ? num : +Math.round(+insert.join('')) + '0'.repeat(pow);
};
