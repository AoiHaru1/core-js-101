 ...
 */
const extractNameFromTemplate = (value) => value.slice(value.indexOf(',') + 2, value.indexOf('!'));
...
const getRectangleString = (width, height) => {
  ...
  for (let i = 1; i <= height; i += 1) {
    if (i === 1) ans += `┌${'─'.repeat(width - 2)}┐\n`;
    if (i === height) ans += `└${'─'.repeat(width - 2)}┘\n`;
    if (i !== 1 && i !== height) ans += `│${' '.repeat(width - 2)}│\n`;
  }
  return ans;
};
/**
...
const encodeToRot13 = (str) => {
  ...
  return str.split('').map((x) => encryptAlphabet[alphabet.indexOf(x)] || x).join('');
};
