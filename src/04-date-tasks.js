...
function timeSpanToString(startDate, endDate) {
  ...
  const lengthChecker = (num) => (num.toString().length === 1 ? `0${num}` : `${num}`);
  const milliSecondsChecker = (num) => {
    if (num.toString().length === 1) return `00${num}`;
    if (num.toString().length === 2) return `0${num}`;
    return `${num}`;
  };
  return `${lengthChecker(hours)}:${lengthChecker(minutes)}:${lengthChecker(seconds)}.${milliSecondsChecker(milliSeconds)}`;
}
...
const angleBetweenClockHands = (date) => {
  const angle = Math.abs((0.5 * ((60 * date.getUTCHours()) - (11 * date.getUTCMinutes()))) % 360);
  // eslint-disable-next-line no-mixed-operators
